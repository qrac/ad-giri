...
現在のサイトでアイコンを押すと、そのサイトをホワイトリストへ追加してブロックを解除します。灰色のアイコンをもう一度押すとブロックを再開します。
...
| `activeTab`             | permissions | ユーザーがaction iconをクリックしたタブで、サイト単位の切り替えに必要なhostnameを一時的に確認し、アイコン状態を更新するため。ページ本文は読み取らない。 |
...
| ------- | ---------- | ---------------------- | ------ |
| 1.0.1   | 2026-08-16 | `tabs` 権限を `activeTab` に置き換え、全タブのアイコン常時同期を廃止 | Draft  |
